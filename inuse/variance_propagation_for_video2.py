...
def main():
    ...
    r = np.array([0., 1.5, 0.])     # 1m height
    v = np.array([0., -4, 6])   # 5 m/s forward (Z), 2 m/s down (-Y)
    ...
    sequence = [
        ...
        {'type': 'flight', 'dt': 0.34},
        {'type': 'bounce', 'normal': normalize(np.array([0, 1, 0])), 'e_n': 0.7069, 'is_curved': False},
        
        # Bounce 2: Curved Surface
        {'type': 'flight', 'dt': 0.63},
        {'type': 'bounce', 'normal': normalize(np.array([0, 1, 0])), 'e_n': 0.7069, 'is_curved': True, 'curvature_radius':-20}, # Flat in original? Check.
